...
// TODO: this should be cloned so that it doesn't mutate the referenced object
...
function _disassociateDocs( rows, schema ) {
    return rows
        .map(function( row ) {
            var doc = row.doc;

            if ( doc.links && doc.links[ schema ] ) {
                delete doc.links[ schema ];
            }
...
function _associateDocs( rows, schema, reference ) {
    return rows
        .map(function( row ) {
            var doc = row.doc;
...
function _updateManyToOne( relatedRespository, resource, reference, field ) {
    ...
    return new RSVP.Promise(function (resolve, reject) {
        ...
        var associate = function( doc, emit ) {
            if ( resource.links !== undefined && ~resource.links[ reference.path ].indexOf( doc._id ) ) {
                emit( doc );
            }
        };
            ...
            .then(function( related ) {
                if (related.rows.length > 0) {
                    return relatedRespository
                        .bulkDocs( _disassociateDocs.call( that, related.rows, field.path ) );
                }
            })
            .then(function() {
                return relatedRespository
                    .query({ map: associate }, options);
            })
            .then(function( related ) {
                if (related.rows.length > 0) {
                    return relatedRespository
                        .bulkDocs( _associateDocs.call( that, related.rows, field.path, resource._id ) )
                        .then( resolve );
                }
