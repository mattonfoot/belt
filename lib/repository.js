...
Repository.prototype._init = function( db ) {
    var dbtype = typeof db;

    this.db = dbtype === 'string' ? new PouchDB( db, this._options ) : db;
...
function _updateOneToOne( relatedModel, resource, reference, field ) {
  ...
  function dissociate( related, emit ) {
  ...
  return new Promise(function (resolve, reject) {
    relatedModel.query({ map: dissociate }, options, function ( error, responses ) {
        if (error) return reject(error);
