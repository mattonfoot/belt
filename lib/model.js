...
Wrapper.prototype.post = function( data, cb ) {
  ...
  Wrapper.prototype.query = function( mapQuery, options, cb ) {
...
function watchForDbChanges( wrapper, options ) {
  wrapper.db
    .changes( options )
    .on('create', function( ev ) {
      var doc = ev.doc;

      wrapper.docs[ doc._id ] = doc;

      wrapper.emit( 'created', doc );
    })
    .on('update', function( ev ) {
      ...
      wrapper.emit( 'updated', doc );
    })
    .on('delete', function( ev ) {
      var doc = ev.doc;

      delete wrapper.docs[ doc._id ];

      wrapper.emit( 'deleted', doc );
    });
}
